pipeline {
    agent any
     environment {
        registry = "529088272063.dkr.ecr.us-west-2.amazonaws.com/python-repo"
    }
    ...
    stages {
          stage('Checkout') {
           ...
           stage('Building image') {
             steps{
                  script {
                      dockerImage = docker.build('529088272063.dkr.ecr.us-west-2.amazonaws.com/python-repo')
                   }
      }
           }
    ...
    sh 'aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin 529088272063.dkr.ecr.us-west-2.amazonaws.com'
     sh 'docker push 529088272063.dkr.ecr.us-west-2.amazonaws.com/python-repo:latest'
}
            ...
            stage('Docker Run') {
              steps{
                   script {
                sh 'docker run -d -p 8096:5000 --rm --name mypythonContainer 529088272063.dkr.ecr.us-west-2.amazonaws.com/python-repo:latest'     
      }
    }
        }
    }
